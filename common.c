...
const char *get_path_to_datafile(const char *path, const char *file_name) {
  static char path_to_file[PATH_MAX]; // to do: malloc
  ...
  const size_t path_without_exe_len = path[i] == '/' ? i + 1 : i;

  if (PATH_MAX < (path_without_exe_len + file_name_len + 1))
    return NULL;

  memset(path_to_file, 0, sizeof(path_to_file));

  strncpy(path_to_file, path, path_without_exe_len);

  strncat(path_to_file, file_name, file_name_len);
